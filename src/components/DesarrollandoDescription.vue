        ...
        .card-carousel-cards(:style="{ transform: 'translateX' + '(' + currentOffset + 'px' + ')'}")
          .card-carousel--card(v-for="(practice, index) in practices")
            button(class="button-practices" @click="setItem(index)"  :class="{isActive: practice.name == itemFocus.name}" data-toggle="tooltip" data-placement="top" :title="practice.name" )
              img(src="/img/desarrollando/carpeta.png" v-if="practice.name == itemFocus.name" )
              img(src="/img/folder_blank.png" v-else )

              span(v-if="practice.name.length > 9") {{practice.name.substring(0,8).toUpperCase()}}...
              span(v-else) {{practice.name.toUpperCase()}}

    .card-carousel--nav__right(
      @click="moveCarousel(1)"
      :disabled="atEndOfList")
  DesarrollandoContent(:data="itemFocus")
...
<script>
  ...
  import DesarrollandoContent from './DesarrollandoContent'

  export default {
    name: "Practices",
    components: {
      DesarrollandoContent,
    ...
    computed: {
      ...
      ...mapState({
        ...
        practices: state => state.desarrollando
      }),
      atEndOfList() {
